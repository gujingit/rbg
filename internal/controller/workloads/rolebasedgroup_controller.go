...
func (r *RoleBasedGroupReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if rbg.Spec.Coordination != nil {
		r.reconcileCoordination(ctx, r, rbg)
	}
			...
			if strategy.UpdateStrategy != nil {
				...
				role.RolloutStrategy = &workloadsv1alpha1.RolloutStrategy{
					...
					RollingUpdate: &workloadsv1alpha1.RollingUpdate{
						MaxUnavailable: strategy.UpdateStrategy.MaxUnavailable,
					},
				}
				...
				roleCoordinationState.State = "Partition: "
...
func isCoordinationRoleCompleted() bool {
	// for role 获取parition，根据label获取pod，判断ready状态是否满足要求
}
